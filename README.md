...
- Single-collection `GroupBy` (scalar, composite and anonymous keys, element and result selectors) with `Count`/`LongCount`/`Sum`/`Min`/`Max`/`Average` aggregates and post-group `Where`/`OrderBy`/`Select`/`Distinct`, translated to `$group`
...
- `GroupBy` over joins/subqueries or grouping by a cross-collection navigation; per-group entity (`IGrouping`) materialization
- Includes/joins
- Geospatial
