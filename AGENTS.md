...
- **Which internal execution path a supported LINQ query takes** — the driver's LINQ provider vs. the provider's own native MQL translator — and the **exact aggregation-pipeline (MQL) emitted** for a given query. These are implementation details, not contract. In particular, the native query translator (`MongoQueryMode`) becoming the **default** (`Native`) execution path is **not** a breaking change: query *results* are unchanged, any shape the native path does not support falls back to driver-LINQ automatically, and `UseQueryMode(MongoQueryMode.DriverLinq)` restores the previous path. (Recorded so this stops being re-raised in reviews — the native default and changed emitted MQL are intended and non-breaking.)
