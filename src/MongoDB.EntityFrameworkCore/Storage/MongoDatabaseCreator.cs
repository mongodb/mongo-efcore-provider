    ...
    : IMongoDatabaseCreator
{
        ...
        foreach (var collectionName in designTimeModel.Model.GetCollectionNames())
        {
            ...
            do
            {
                ...
                foreach (var indexModel in cursor.ToList())
                {
                    ...
                    if (status == "FAILED")
                    {
                        throw new InvalidOperationException(BuildIndexFailureMessage(indexModel, isVector));
                    }
    ...
    private static string BuildIndexFailureMessage(BsonDocument indexModel, bool isVector)
    {
        var indexName = indexModel.GetValue("name", (BsonValue)"<unknown>");
        if (!isVector)
        {
            return $"Failed to build the search index '{indexName}'.";
        }

        var path = TryGetVectorIndexPath(indexModel);
        return path == null
            ? $"Failed to build the vector index '{indexName}'."
            : $"Failed to build the vector index '{indexName}' for path '{path}'.";
    }

    private static string? TryGetVectorIndexPath(BsonDocument indexModel)
        => indexModel.TryGetValue("latestDefinition", out var definition)
           && definition is BsonDocument definitionDoc
           && definitionDoc.TryGetValue("fields", out var fields)
           && fields is BsonArray { Count: > 0 } fieldsArray
           && fieldsArray[0] is BsonDocument firstField
           && firstField.TryGetValue("path", out var path)
            ? path.ToString()
            : null;
        ...
        foreach (var collectionName in designTimeModel.Model.GetCollectionNames())
        {
            ...
            do
            {
                ...
                foreach (var indexModel in indexModels)
                {
                    ...
                    if (status == "FAILED")
                    {
                        throw new InvalidOperationException(BuildIndexFailureMessage(indexModel, forVectors));
                    }
