...
- **vs Query — bulk `ExecuteUpdate`/`ExecuteDelete` (EF9+).** `MongoBulkOperationExecutor` runs server-side bulk writes (`deleteMany`/`updateMany`, single-command or transactional two-phase) from a `MongoBulkPlan` the Query pipeline builds. Unlike the read seam (which crosses as data), the plan carries `Func<QueryContext, …>` translation delegates that the executor invokes at runtime — so a translation failure surfaces from a Storage frame (`TranslateBulkOrThrow` maps it to EF's canonical "could not be translated"). The executor resolves `IMongoClientWrapper`/`IDiagnosticsLogger` from the `QueryContext`; it does **not** translate.
- **Two transaction-orchestration paths exist — keep them in sync.** `MongoDatabaseWrapper.SaveChanges` starts its implicit transaction directly via `MongoTransaction.Start` (and honors `AutoTransactionBehavior.WhenNeeded`, count-based). The bulk two-phase path (`MongoBulkOperationExecutor.RunInBulkTransaction`) goes through `database.BeginTransaction()` (so phase-1 reads see `Database.CurrentTransaction`) and always begins for two-phase; it maps standalone-deployment failures to an actionable error via `IsTransactionsUnsupported`. A change to transaction policy (a new `AutoTransactionBehavior`, retry, standalone handling) must be applied to **both**.
...
- **RowVersion increments before serialization.** If serialization fails, the in-memory value is already advanced. Callers should expect that on retry-after-failure they may need to re-read the entity. The ordering inside `MongoUpdate.ConvertModified` is `CreateWhereFilter` (captures original concurrency-token values) → `SetStoreGeneratedValues` (increments the in-memory row version) → `WriteEntity` (serializes the post-increment value into the `$set` update); reordering breaks the contract that the WHERE clause matches the pre-increment value while the `$set` writes the post-increment one.
