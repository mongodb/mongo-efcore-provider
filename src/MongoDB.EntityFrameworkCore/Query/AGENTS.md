...
IQueryable<T>  (EF Core)
   ...
   │      └─ rejects unsupported shapes (Join / set ops) early; single-collection GroupBy translates to $group
...
- **Unsupported shapes are detected, not silently translated.** Joins and set operations (`Intersect` / `Except`) throw early — see the early-fail branches in `MongoQueryableMethodTranslatingExpressionVisitor`. Single-collection `GroupBy` is translated (`TranslateGroupBy` builds a `GroupByShaperExpression`, applying element and result selectors); GroupBy over a join/lookup source returns `null` so EF reports "could not be translated", and per-group entity (`IGrouping`) materialization is rejected downstream in shaping.
