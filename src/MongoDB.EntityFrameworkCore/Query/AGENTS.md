...
> **As-built scope (EF-323 foundation + EF-329 predicate-breadth sub-project).** Native = filter/sort/paging, now with **predicate breadth**: nullable equality/`== null` (IS NULL), collection `Contains` → `$in`/`$nin`, `string.StartsWith`/`EndsWith`/`Contains` → `$regularExpression`, and field-to-field/arithmetic comparisons → `$expr`. **Reference `Include` is deferred** — it nav-expands to a `LeftJoin` that the gate treats as non-native, so it falls back to driver-LINQ (correct results; throws under `NativeOnly`). The native lookup machinery (lowerer `$lookup`/`$unwind`, `GetStreamingReferenceLookups`, streaming-Include eligibility) is **built but dormant**, awaiting the Collection Includes sub-project. The remaining **computed long tail** — string transforms (`ToUpper`/`Substring`/…), date-part extraction, `Math.*`, and type-changing casts — is explicitly **out of scope for this sub-project**; it is a follow-on, not a gap left by predicate breadth. Projection pushdown, scalar cardinality, `GroupBy`/set ops/`Distinct`/`OfType`, etc. remain later sub-projects.
...
- **Native translation (`NativeTranslation/` + `Expressions/`):**
  ...
  - `Expressions/MongoSelectDefinition` — the native-translation logical query IR (`Predicate`/`Orderings`/`Offset`/`Limit`/`IsNativeRepresentable`), extracted (EF-330) from `MongoQueryExpression` into a composed, dialect-neutral data-holder. The QMTEV populates it via `mongoQ.Select`; the gate and lowerer read it. The `$lookup` state stays on `MongoQueryExpression` (entangled with the fallback shaper, slated for EF-317), so it was **not** moved here.
