...
// Originally EF Core ProjectionMember.cs
...
/// </remarks>
public sealed class MongoProjectionMember
{
    private readonly IList<string> _memberChain;

    /// <summary>
    ///     Creates a new instance of the <see cref="MongoProjectionMember" /> class with empty MemberInfo chain.
    /// </summary>
    public MongoProjectionMember()
        => _memberChain = new List<string>();

    private MongoProjectionMember(IList<string> memberChain)
    ...
    public MongoProjectionMember Append(string member)
    {
        var existingChain = _memberChain.ToList();
        existingChain.Add(member);

        return new MongoProjectionMember(existingChain);
    }
    ...
    public MongoProjectionMember Prepend(string member)
    {
        var existingChain = _memberChain.ToList();
    ...
    public override bool Equals(object? obj)
        => obj is MongoProjectionMember projectionMember && Equals(projectionMember);

    private bool Equals(MongoProjectionMember other)
    {
