...
/// </remarks>
public class StopWordFilterDefinition : SearchIndexDefinitionBase
{
    /// <summary>
    /// List of the stop words that correspond to the tokens to remove.
    /// </summary>
    public IReadOnlyList<string> Tokens { get; set; } = null!;

    /// <summary>
    /// If <see langword="true"/>, then matching of tokens is case-insensitive, otherwise token matching is case-sensitive.
    /// </summary>
    ...
    {
        var document = new BsonDocument { { "type", Name }, { "tokens", new BsonArray(Tokens) } };
        document.Add("ignoreCase", IgnoreCase, IgnoreCase != null);
        return document;
    }
}
