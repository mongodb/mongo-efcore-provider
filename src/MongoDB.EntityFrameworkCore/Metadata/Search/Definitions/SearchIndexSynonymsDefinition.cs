...
/// </remarks>
public class SearchIndexSynonymsDefinition : SearchIndexDefinitionBase
{
    private readonly List<ISearchIndexDefinition> _synonymDefinitions = new();

    /// <summary>
    /// Finds the definition for the given synonym mapping name. If none is found, then a new instance is created and returned.
    /// </summary>
    /// <param name="synonymsName">The name of the synonym mapping.</param>
    ...
    public SearchIndexSynonymDefinition GetOrAddSearchIndexSynonymDefinition(string synonymsName)
        => _synonymDefinitions.GetOrAddDefinition<SearchIndexSynonymDefinition>(synonymsName);

    /// <inheritdoc/>
    public override BsonValue ToBson()
    {
        var array = new BsonArray();
        foreach (var synonymDefinition in _synonymDefinitions)
        {
            array.Add(synonymDefinition.ToBson());
        }
