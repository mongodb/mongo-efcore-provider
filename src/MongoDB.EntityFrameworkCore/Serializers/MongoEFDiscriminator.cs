    ...
    : IDiscriminatorConvention
{
    public Type GetActualType(IBsonReader bsonReader, Type nominalType)
    {
        ...
        try
        {
            if (bsonReader.FindElement(ElementName))
            {
                ...
                var actualEntityType = entityType.Model.GetEntityTypes()
                    .FirstOrDefault(e => Equals(discriminator, e.GetDiscriminatorValue()));
                if (actualEntityType != null)
                {
                    actualType = actualEntityType.ClrType;
                }
            }
        }
        finally
    ...
    public BsonValue GetDiscriminator(Type nominalType, Type actualType)
    {
        ...
        return BsonString.Create(childEntityType.GetDiscriminatorValue());
    }
