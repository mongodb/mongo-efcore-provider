...
- `ValueConverterSerializer<TActual, TStorage>` — wraps an EF `ValueConverter<TActual, TStorage>` around a storage serializer. The C# driver knows nothing about EF; this is the adapter. Nullable model types throw — see the explicit guard in `BsonSerializerFactory.CreateValueConverterSerializer`.
