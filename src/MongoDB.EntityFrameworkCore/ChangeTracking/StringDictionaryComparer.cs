...
// Originally from EFCore.Cosmos StringDictionaryComparer.cs in EF10
...
public sealed class StringDictionaryComparer<TDictionary, TElement> : ValueComparer<object>, IInfrastructure<ValueComparer>
{
    ...
    private static bool Compare(object? a, object? b, Func<TElement?, TElement?, bool> elementCompare)
    {
        ...
        throw new InvalidOperationException(FormatBadDictionaryTypeMessage(
    ...
    private static string FormatBadDictionaryTypeMessage(object a, object b)
        => $"The type '{a}' cannot be mapped as a dictionary because it does not implement '{b}'.";
    ...
    private static int GetHashCode(IEnumerable source, Func<TElement?, int> elementGetHashCode)
    {
        ...
        {
            throw new InvalidOperationException(FormatBadDictionaryTypeMessage(source.GetType().ShortDisplayName(),
